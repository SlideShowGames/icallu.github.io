pyro.createStage({
	...
	onCreate: function(){
		...
		this.input.bind({
			...
			callback: function(stage, events){
                if(stage.selectedChoice != stage.layers[0].getObjectsByClass("Choice").indexOf(events[0].entity)) pyro.getStage("Gameplay").audio.play({id: "select"});
