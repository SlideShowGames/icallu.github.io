...
pc.script.create('particle_emitter', function (context) {
    ...
    Particle_emitter.prototype = {
        ...
        reset: function () {
            ...
            if (this.colorMap) {
                var colorMap = context.assets.get(this.colorMap);
                colorMap.ready(function (asset) {
                    this.emitter.setColorMap(asset.resource);
                }.bind(this));
                context.assets.load(colorMap);
            }

            if (this.opacityMap) {
                var opacityMap = context.assets.get(this.opacityMap);
                opacityMap.ready(function (asset) {
                    this.emitter.setOpacityMap(asset.resource);
                }.bind(this));
                context.assets.load(opacityMap);
            }
        },
